...
class BaseViewController: UIViewController,UINavigationControllerDelegate, UIGestureRecognizerDelegate {
    private let HiddenNavigationBarViewControllersStringName:[String] = ["DLJobViewController","JobSearchViewController","DLMeViewController"]
    ...
    func setLeftNavigationBackItem(){
        ...
        leftButton.setImage(UIImage(named: "back_image_black"), for: UIControl.State.normal)
        leftButton.addTarget(self, action: #selector(clickedLeftNavigationItem), for: UIControl.Event.touchUpInside)
    ...
    func setRightNavgationItemWithImage(imageName:String){
        self.setRightNavgationItem(title: nil, image: UIImage(named: imageName))
    }
